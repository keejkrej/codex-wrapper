...
export function createWsNativeApi(): NativeApi {
  ...
  const api: NativeApi = {
    dialogs: {
      pickFolder: async () => {
        if (!(window as DesktopWindow).desktopBridge) return null;
        return (window as DesktopWindow).desktopBridge!.pickFolder();
      },
      confirm: async (message) => {
        if ((window as DesktopWindow).desktopBridge) {
          return (window as DesktopWindow).desktopBridge!.confirm(message);
        }
    ...
    shell: {
      ...
      openExternal: async (url) => {
        if ((window as DesktopWindow).desktopBridge) {
          const opened = await (window as DesktopWindow).desktopBridge!.openExternal(url);
          if (!opened) {
    ...
    contextMenu: {
      show: async <T extends string>(
        ...
        if ((window as DesktopWindow).desktopBridge) {
          return (window as DesktopWindow).desktopBridge!.showContextMenu(
            items,
            position,
          ) as Promise<T | null>;
        }
