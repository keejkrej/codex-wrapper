...
type DesktopWindow = Window & { desktopBridge?: import("@t3tools/contracts").DesktopBridge };
...
const isWsPushMessage = (value: WsResponseMessage): value is WsPush =>
  typeof value === "object" && value !== null && "type" in value && value.type === "push";
...
export class WsTransport {
  ...
  constructor(url?: string) {
    const bridgeUrl =
      typeof (window as DesktopWindow).desktopBridge?.getWsUrl === "function"
        ? (window as DesktopWindow).desktopBridge!.getWsUrl()
        : null;
    const envUrl = env.VITE_WS_URL;
    const locationProtocol =
      typeof window.location?.protocol === "string" ? window.location.protocol : "http:";
    const locationHost =
    ...
    this.url =
      ...
      (bridgeUrl && bridgeUrl.length > 0
        ...
        : envUrl && envUrl.length > 0
          ? envUrl
          : `${locationProtocol === "https:" ? "wss" : "ws"}://${locationHost}`);
    this.connect();
  }
  ...
  private handleMessage(raw: unknown) {
    ...
    if (Result.isFailure(result)) {
      console.warn(
        ...
        formatSchemaError(
          (result as unknown as { failure: Parameters<typeof formatSchemaError>[0] }).failure,
        ),
      );
      return;
    }

    const message = (result as unknown as { success: WsResponseMessage }).success;
